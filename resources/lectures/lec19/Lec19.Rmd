...
\vspace{-6mm}
$$
y_t = 0.05 + 0.65 y_{t-1} + 0.25 y_{t-2} + \epsilon_t
$$
:::
...
y[2] <- 2 + 0.65*y + eps[2]
for(i in 3:102){
  y[i] <- 2 + 0.55*y[i-1] + 0.25*y[i-2] + eps[i]
}
...
\vspace{-2mm}
 ...
 - One to take in inputs of:
   ...
   - $\phi_1$
   - $\phi_2$
   - and then output a dataframe of `y` and `time`. You can hardcode `c=0.005` and `sd=10`.
...
\vspace{-2mm}
to run a simulation study of 1000 reps with $n=100, \theta_1=0.65, \theta_2=0.25$ and compare power with:
...
\vspace{-6mm}
$$
y_t = \beta_0 + \beta_1 x + \phi_1 y_{t-1} + \phi_2 y_{t-2} + \epsilon_t
$$
:::
